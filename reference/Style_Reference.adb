...
package body Neo.World.Graphics is
      ...
      begin
        for I of Stages loop
          if Has_Stage (I.Kind) then
            Error ("When loading shader " & Path & " multiple " & I.Kind'Wide_Image & "'s were found");
            raise Program_Error;
          end if;
  ...
  package body Sampler is
      ...
      procedure Set (Val : Str) is
        ...
        begin
          Writes.Append (Write_Descriptor_State'
                          (Is_Image   => True,
  ...
  package body Uniform is   
          ...
          Writes.Append (Write_Descriptor_State'
            (Is_Image    => False,
             Buffer_Info => (buffer => Buffer.Data,
                             ...
                             rang   => Uniform_T'Object_Size / Byte'Size),
             Set         => (dstBinding      => Binding,
                             descriptorCount => 1,
    ...
    begin
      Buffered_Uniforms.Insert (U (Name_ID), (Usage   => Usage,
                                              Kind    => Uniform_Buffer,
                                              Binding => Binding,
                                              Size    => Uniform_T'Object_Size / Byte'Size, others => <>));
  ...
  package Base_Color_Sampler is new Sampler (Binding => 0);
  package Irradiance_Sampler is new Sampler (Binding => 1);
  package Filter_Sampler     is new Sampler (Binding => 2);
  package Specular_Sampler   is new Sampler (Binding => 3);
  package Normal_Sampler     is new Sampler (Binding => 4);
  package Displace_Sampler   is new Sampler (Binding => 5);
  package Metallic_Sampler   is new Sampler (Binding => 6);
  package Roughness_Sampler  is new Sampler (Binding => 7);
  package Projection_Sampler is new Sampler (Binding => 8);
  package Falloff_Sampler    is new Sampler (Binding => 9);
  package Menu_Sampler       is new Sampler (Binding => 10);
  
  -- Skinning flag
  package Enable_Skinning    is new Uniform (Binding => 11, Uniform_T => Bool, Name_ID => "enableSkinning");
  
  -- Texture map flags
  package Enable_Base_Color  is new Uniform (Binding => 12, Uniform_T => Bool, Name_ID => "enableBaseColor");
  package Enable_Irradiance  is new Uniform (Binding => 13, Uniform_T => Bool, Name_ID => "enableIrradiance");
  package Enable_Specular    is new Uniform (Binding => 15, Uniform_T => Bool, Name_ID => "enableSpecular");
  package Enable_Normal      is new Uniform (Binding => 16, Uniform_T => Bool, Name_ID => "enableNormal");
  package Enable_Metallic    is new Uniform (Binding => 18, Uniform_T => Bool, Name_ID => "enableMetallic");
  package Enable_Roughness   is new Uniform (Binding => 19, Uniform_T => Bool, Name_ID => "enableRoughness");
  ...
  procedure Run_Frontend is
    ...
    begin
      loop
        ...
        -- Build and set the view for use by the backend
        Renderer.Build_View (Current);
        View.Set (Current);
        
        -- Clear the view
        Current.Surfaces.Clear;
        for Light of Current.Lights loop
          Light.Clear;
        end loop;
        Current := (others => <>);
  ...
  procedure Run_Backend is 
    ...
    -- Signal the main task to perform a restart and give it some time
    procedure Trigger_Framebuffer_Restart is
      begin
        Framebuffer_Status.Occupied (False);
        delay 10.0;
        
        -- We shouldn't reach here since a restart would entail finalizing the backend task...
        raise Program_Error;
      end;
    ...
    -- Buffer settings for frame commands
    Command_Buffer_Begin_Info : aliased VkCommandBufferBeginInfo := (flags => VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT, others => <>);
    
    -- Render pass settings
    Clear_Values           : aliased Array_VkClearValue    := (1..2 => (color => (0.0, 0.0, 0.0, 1.0), depthStencil => (1.0, 0)));  
    Render_Pass_Begin_Info : aliased VkRenderPassBeginInfo := (clearValueCount => 2,
    ...
    -- Frame command submission settings
    ...
    begin
      loop
        ...
        vkAssert (vkFreeDescriptorSets (Device, Descriptor_Pool, 1, Framebuffer (I).Descriptor_Set'Unchecked_Access));    
        ...
        Renderer.Build_Frame (Framebuffer (I), View.Get);
        ...
        vkAssert (Safe_Queue.vkQueueSubmit (1, Submit_Info'Unchecked_Access, Framebuffer (I).Fence));
  
        -- Show the resulting image
        Present_Info.pWaitSemaphores := Framebuffer (I).Render_Status'Unchecked_Access;
        if Safe_Queue.vkQueuePresent (Present_Info'Unchecked_Access) in VK_ERROR_OUT_OF_DATE_KHR | VK_SUBOPTIMAL_KHR then
  ...
  protected body Safe_Queue is 
      function vkQueueSubmit (submitCount : Int_Unsigned_C; pSubmits : Ptr_VkSubmitInfo; fence : Ptr) return Int_Unsigned_C
                              is (Neo.API.Vulkan.vkQueueSubmit (Queue, submitCount, pSubmits, Fence));
      function vkQueuePresent (pPresentInfo : Ptr_VkPresentInfoKHR) return Int_Unsigned_C
                               is (vkQueuePresentKHR (Queue, pPresentInfo));
    end;
  ...
  -- Create and stage a new buffer based on usage
  procedure Initialize_Buffer (Buffer     : in out Buffer_State;
                               Usage_Bits : Int_Unsigned_C;
                               Data       : Ptr;
                               Data_Size  : Int_Ptr;
                               Count      : Positive)
    is
    Buffer_Info : aliased VkBufferCreateInfo := (size        => Int_64_Unsigned_C (Data_Size / Byte'Size),
  ...
  -- Set the staging buffer and submit the commands so the transfer happens within the protected operation
  procedure Set_Buffer (Buffer : in out Buffer_State; Data : Ptr; Data_Size : Int_Ptr) is
    begin
      case Buffer.Usage is
      
        -- Move data from CPU host visible memory to GPU device memory
        when GPU_Usage =>
        
          -- Sanity checks
          Debug_Assert (Data_Size <= Max_Upload_Buffer.Get * MB);
          
          -- Copy it in
          declare                
pragma Warnings (Off); -- warnings: possible unprotected access to protected data
          Submit_Info : aliased VkSubmitInfo :=  (commandBufferCount => 1,
...
pragma Warnings (On);   
          ...
          Memory_Range              : aliased VkMappedMemoryRange      := (size   => VK_WHOLE_SIZE, 
                                                                           memory => Int_64_Unsigned_C (To_Int_Ptr
                                                                                       (Staging_Device_Memory)),others => <>);                           
          ...
          Buffer_Copy : aliased VkBufferCopy := (size => Int_64_Unsigned_C (Source'Length), others => <>);    
          begin
            Destination := Source;
            ...
            -- Buffer the data
            if Buffer.Kind /= Image_Buffer then
              ...
              Image_Memory_Barrier : aliased VkImageMemoryBarrier :=
                (oldLayout           => VK_IMAGE_LAYOUT_UNDEFINED,
              ...
              begin
                ...
                -- Transition image layout for transfer to the GPU
                vkCmdPipelineBarrier (commandBuffer            => Staging_Commands,
                ...
                vkCmdCopyBufferToImage (commandBuffer  => Staging_Commands,
                                        srcBuffer      => Staging_Buffer,
                                        dstImage       => Buffer.Data,
                                        dstImageLayout => VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                                        regionCount    => 1,
                                        pRegions       => Buffer_Image_Copy'Unchecked_Access);   
            
                -- Transition image layout for reading from a shader
            ...
            Ignore (Safe_Queue.vkQueueSubmit  (1, Submit_Info'Unchecked_Access, Staging_Fence));
          end;
          
        -- There is nothing to stage when we are on the CPU
        when others => null;
      end case;
    end;
    
  -- Calculate the Vulkan memory flags based on our usage needs
  function Find_Memory_Type_Index (Memory_Type_Bits : Int_Unsigned_C; Usage : Usage_Kind) return Int_Unsigned_C is
    ...
    begin
      ...
      case Usage is
        ...
        when CPU_Usage        => Required  := VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT or VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
        when GPU_To_CPU_Usage => Preferred := VK_MEMORY_PROPERTY_HOST_CACHED_BIT  or VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
                                 Required  := VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT;                                 
      end case;
      ...
      for I in 1..Int (Memory_Properties.memoryTypeCount) loop
        ...
        if (Memory_Type_Bits and 2 ** (I - 1)) /= 0
          and (Preferred = 0 or else (Properties and Preferred) > 0)
          and (Required  = 0 or else (Properties and Required)  > 0)
        then
          return Int_Unsigned_C (I - 1);
        end if;
      end loop;    
        
      -- Couldn't find our ideal memory type, so match on the required usage at least
      if Required /= 0 then
        for I in 1..Int (Memory_Properties.memoryTypeCount) loop
          Properties := Memory_Properties.memoryTypes (I).propertyFlags;
          if (Memory_Type_Bits and 2 ** (I - 1)) /= 0 and (Properties and Required) > 0 then
            return Int_Unsigned_C (I - 1);
          end if;
        end loop;
      end if;
      
      -- Go with preferred usage when we can't even match on required usage
      if Preferred /= 0 then
        for I in 1..Int (Memory_Properties.memoryTypeCount) loop
          Properties := Memory_Properties.memoryTypes (I).propertyFlags;
          if (Memory_Type_Bits and 2 ** (I - 1)) /= 0 and (Properties and Preferred) > 0 then
            return Int_Unsigned_C (I - 1);
          end if;
        end loop;
      end if;
  ...
  -- Protected wrapper for allocating and managing GPU memory (works closely with Safe_Staging)
  protected body Safe_Allocation is 
  
      -- Put something in the garbage to be collected by the backend
      ...
      procedure Allocate_Buffer (Buffer : in out Buffer_State) is
        ...
        function Align (Offset, Alignment : Int_64_Unsigned_C) return Int_64_Unsigned_C is
          ((Alignment - 1 + Offset) and not (Alignment - 1));
        ...
        begin 
          ...
          if Buffer.Kind = Image_Buffer then
            vkGetImageMemoryRequirements (Device, Buffer.Data, Memory_Requirements'Unchecked_Access);
          else
          ...
          begin
            ...
            if Heap.Usage /= GPU_Usage then
              vkAssert (vkMapMemory (Device, Heap.Device_Memory, 0, Memory_Requirements.size, 0, Heap.Data'Address));
            end if;
            Heaps.Append (Heap);
      ...
      procedure Finalize_Buffers (Force_Total_Finalization : Bool := False) is
        package Vector_Hashed_Buffer_Cursor is new Vectors (Hashed_Buffer.Cursor);
        Zombie_Images : Vector_Hashed_Buffer_Cursor.Unsafe.Vector;
        ...
        begin            
          ...
          while Has_Element (I) loop declare Buffered_Image : Buffer_State := Element (I); begin
            if Force_Total_Finalization or Buffered_Image.References = 0 then
              ...
              Zombie_Images.Append (I);
            end if;
            Hashed_Buffer.Unsafe.Next (I);
          end; end loop;
          
          -- Remove zombie images freed from the previous loop (which can be done in-line due to cursor woes)
          for Zombie of Zombie_Images loop Buffered_Images.Delete (Zombie); end loop;
  ...
  procedure Buffer_Mesh (Path : Str; Mesh : Ptr_Mesh_State) is
    Buffered_Mesh : Ptr_Buffered_Mesh_State;
    begin 
      
      -- Bone weights
      if Mesh.Skeleton /= null then
        Buffered_Mesh := new Buffered_Mesh_State'(Is_Animated   => True,
                                                  Segment_Count => Mesh.Segment_Count, others => <>);
--          Initialize_Buffer (Buffer     => Current_Surface.Weights,
--                             Usage_Bits => VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
--                             Data       => Mesh.Bone_Weights (1)'Address,
--                             Data_Size  => (Data'Length * Weight_State'Object_Size) / Byte'Size,
--                             Count      => Mesh.Animated_Surfaces.Element (I).Weights.Length);
      else
        Buffered_Mesh := new Buffered_Mesh_State'(Is_Animated   => False,
                                                  Segment_Count => Mesh.Segment_Count, others => <>);
      end if;  
      
      -- Verticies
      Initialize_Buffer (Buffer     => Buffered_Mesh.Vertices,
                         Usage_Bits => VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                         Data       => Mesh.Vertices (1)'Address,
                         Data_Size  => Mesh.Vertices'Size,
                         Count      => Mesh.Vertex_Count);
      
      -- Mesh segment
      for I in Mesh.Segments'Range loop
        declare
        Mesh_Indicies : Array_x2_Int_32_Unsigned (1..Mesh.Segments (I).Count, 1..3);
        for Mesh_Indicies'Address use Mesh.Indices (Mesh.Segments (I).Index, 1)'Address;
        begin
        
          -- Material
          Buffered_Mesh.Segments (I).Material := U (To_Str_16 (Mesh.Segments (I).Material));
          begin
          Buffer_Material (To_Str_16 (Mesh.Segments (I).Material));
          exception when others => null; end;
  
          -- Indicies            
          Initialize_Buffer (Buffer     => Buffered_Mesh.Segments (I).Indices,
                             Usage_Bits => VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                             Data       => Mesh_Indicies'Address,
                             Data_Size  => Mesh_Indicies'Size,
                             Count      => Mesh.Segments (I).Count * 3);
        end;
      end loop;
      Buffered_Meshes.Insert (Geometry_Name_Str.U (Mesh.Name), Buffered_Mesh);
    end;
    
  -- Free a mesh from GPU memory
  procedure Unbuffer_Mesh (Path : Str) is
    Buffered_Mesh : Ptr_Buffered_Mesh_State := Buffered_Meshes.Get (Path);
    begin      
      Safe_Allocation.Throw_Away (Buffered_Mesh.Vertices);
      for I in Buffered_Mesh.Segments'Range loop
        Remove_Material_Reference (S (Buffered_Mesh.Segments (I).Material));  
        Safe_Allocation.Throw_Away (Buffered_Mesh.Segments (I).Indices);
...
--    procedure Create_Top_Level_Structure is
--      Buffered_Structure : Buffer_State (Structure_Object_Buffer);
--      Acceleration_Structure_Memory_Info : aliased VkBindAccelerationStructureMemoryInfoNV := (others => <>);
--      Acceleration_Structure_Info        : aliased VkAccelerationStructureCreateInfoNV :=
--        (info => (typ           => VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_NV,
--                  instanceCount => 1,
--                  geometryCount => 0, others => <>), others => <>);
--      begin
--        vkAssert (vkCreateAccelerationStructureNV (Device, Acceleration_Structure_Info'Access, null, Buffered_Acceleration.Data'Unchecked_Access));
--     
--        -- Get the special memory requirements   
--        Safe_Allocator.Allocate (Buffered_Structure);
--        Acceleration_Structure_Memory_Info.accelerationStructure := Buffered_Structure.Data;
--        Acceleration_Structure_Memory_Info.memory                := Buffered_Structure.Device_Memory;
--        vkAssert (vkBindAccelerationStructureMemoryNV (Device, 1, Acceleration_Structure_Memory_Info'Unchecked_Access));
--     
--        -- Fetch our sepcial "handle" used to Id the structure somehow ???
--        vkAssert (vkGetAccelerationStructureHandleNV
--          (Device, Buffered_Structure.Data, Unsigned_64_Int'Size, Buffered_Structure.Handle'Unchecked_Access));
--          
--        Buffered_Structures.Append (Buffered_Structure);
--          
--        -- Cache the buffer
--        Buffered_Structures.Insert (Path, Buffered_Structures);
--      end;
--  
--    procedure Buffer_Bottom_Level_Structure (Path : Str) is
--    
--      -- Result and temporaries
--      Buffered_Structures : Vector_Buffer_State (Structure_Object_Buffer);
--      Buffered_Structure  : Buffer_State (Structure_Object_Buffer);
--        
--      -- Geometry
--      Buffered_Mesh : Vector_Buffer_Surface.Unsafe.Vector := Buffered_Meshes.Get (Path);
--      Geometry : aliased VkGeometryNV := (geometryType => VK_GEOMETRY_TYPE_TRIANGLES_NV,
--                                          flags        => VK_GEOMETRY_OPAQUE_BIT_NV,
--                                          geometry     => (triangles => (vertexData   => scene.vertices.buffer,
--                                                                         vertexCount  => scene.vertexCount,
--                                                                         --vertexStride => vertexLayout.stride(),
--                                                                         vertexFormat => VK_FORMAT_R32G32B32_SFLOAT,
--                                                                         indexData    => scene.indices.buffer,
--                                                                         indexCount   => scene.indexCount,
--                                                                         indexType    => VK_INDEX_TYPE_UINT32, others => <>), others => <>), others => <>);
--                                                                         
--      
--      Acceleration_Structure_Memory_Info : aliased VkBindAccelerationStructureMemoryInfoNV := (others => <>);
--      Acceleration_Structure_Info        : aliased VkAccelerationStructureCreateInfoNV :=
--        (pGeometries => geometries,
--         info        => (typ           => VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_NV,
--                         instanceCount => 0,
--                         geometryCount => 1, others => <>), others => <>);
--                                                                                    
--      -- Start of Create_Bottom_Level_Acceleration
--      begin    
...
--      end;
  ...
  function Get_Material_Info_Hash (Item : Material_State) return Str_Unbound is
    (U (Game.Filter_Kind'Pos (Item.Filter)'Wide_Image & Clamp_Kind'Pos (Item.Clamp)'Wide_Image));
  ...
  procedure Buffer_Material (Path : Str) is
    ...
    procedure Buffer_Image (Path : Str) is
      Image : Ptr_Image_State;
      begin
      
        -- Empty paths mean no image to be buffered
        if Path'Length = 0 then return; end if;
  
        -- Increment the count if it is loaded already
        if Buffered_Images.Has (Path & Hash) then Add_Image_Reference (Path & Hash); return; end if;
        
        -- Load the texture file to local memory
        begin
        Load (S (OS_Info.App_Path) & PATH_TEXTURES & Path, Image);
        exception when others => return; end;
        
        -- Buffer it to Vulkan
        declare
        Buffered_Image  : Buffer_State := (Kind   => Image_Buffer, Size => Int_64_Unsigned (Image.Data_Size),
                                           Width  => Int_Unsigned_C (Image.Width),
                                           Height => Int_Unsigned_C (Image.Height), others => <>);
        ...
        Image_Info : aliased VkImageCreateInfo := (initialLayout => VK_IMAGE_LAYOUT_UNDEFINED,
                                                   ...
                                                   extent        => (Int_Unsigned_C (Image.Width),
                                                                     Int_Unsigned_C (Image.Height), 1, others => <>),
                                                   arrayLayers   => Image.Faces,
                                                   flags         => (if Image.Faces = 6 then VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT
                                                                                        else 0),
                                                   usage         => VK_IMAGE_USAGE_TRANSFER_DST_BIT
                                                                      or VK_IMAGE_USAGE_SAMPLED_BIT, others => <>); -- Added!
        begin
          ...
          -- Set the image buffer to the loaded image
          Set_Buffer (Buffered_Image, Image.Data (1)'Address, Int_Ptr (Image.Data'Size));
          
          -- Register the buffered image
          Buffered_Images.Insert (Path & Hash, Buffered_Image);
        end;
        
        -- Free the texture in local memory
        Free (Image);
      end;
      
    -- Start of Buffer_Material
    begin
    
      
      if not Materials.Has (Path) then
        Info ("Missing material " & Path);
        return;
      end if;
      Material := Materials.Get (Path);
      Hash     := Get_Material_Info_Hash (Material);
        
      -- Sanity check
      if Buffered_Materials.Has (Path) then
        --  Line ("Attempted to rebuffer material " & Path);
        return;
      end if;

      -- Buffer the material and load all images associated with the material to the GPU
      Buffer_Image (To_Str_16 (Material.Irradiance));
      Buffer_Image (To_Str_16 (Material.Specular));
      Buffer_Image (To_Str_16 (Material.Normal));
      Buffer_Image (To_Str_16 (Material.Base_Color));
  ...
  procedure Unbuffer_Material (Path : Str) is
    use Texture_Name_Str;
    Material : Material_State := Materials.Get (Path);
    Hash     : Str_Unbound    := Get_Material_Info_Hash (Material);
    begin
  
      -- Make sure we can't unbuffer a material until all buffered surfaces using it get unbuffered first
      if Buffered_Images.Get (Path).References > 0 then Warn ("Unbuffering referenced material?"); end if; 

      -- Decrement the image's usage count or deallocate the image
      if Material.Base_Color.Current_Length = 0 then Remove_Image_Reference (S (Material.Base_Color) & Hash); end if;
      if Material.Irradiance.Current_Length = 0 then Remove_Image_Reference (S (Material.Irradiance) & Hash); end if;
      if Material.Specular.Current_Length   = 0 then Remove_Image_Reference (S (Material.Specular)   & Hash); end if;
      if Material.Normal.Current_Length     = 0 then Remove_Image_Reference (S (Material.Normal)     & Hash); end if;
      if Material.Metallic.Current_Length   = 0 then Remove_Image_Reference (S (Material.Metallic)   & Hash); end if;
      if Material.Roughness.Current_Length  = 0 then Remove_Image_Reference (S (Material.Roughness)  & Hash); end if;
      
      -- Clear the material from the GPU buffered list
      Buffered_Images.Delete (To_Str_16 (Material.Base_Color) & To_Str_16 (Hash));
  ...
  -- Initiate a restart of the current framebuffer if we detect a resize
  ...
  -- Finalize the framebuffer and everything it uses
  procedure Finalize_Framebuffer is
    begin
      Backend_Task.Finalize;
      Frontend_Task.Finalize;
  ...
  -- Initialize the framebuffer - this may happen multiple times during resizing
  procedure Initialize_Framebuffer is
    ...
    Swapchain_Info : aliased VkSwapchainCreateInfoKHR := (surface          => Surface,
                                                          ...
                                                          minImageCount    => Surface_Details.maxImageCount,
                                                          preTransform     => Surface_Details.currentTransform,
                                                          imageExtent      => Surface_Details.currentExtent, others => <>);
   
                                                          
    -- Command buffer for transitioning depth buffer
    Command_Buffer               : aliased Ptr;
    Command_Buffer_Begin_Info    : aliased VkCommandBufferBeginInfo    := (flags => VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT, others => <>);
    Command_Buffer_Allocate_Info : aliased VkCommandBufferAllocateInfo := (commandPool        => Command_Pool,
                                                                           commandBufferCount => 1,
                                                                           level              => VK_COMMAND_BUFFER_LEVEL_PRIMARY, others => <>);
    Submit_Info : aliased VkSubmitInfo := (commandBufferCount => 1,
                                           pCommandBuffers    => Command_Buffer'Unchecked_Access, others => <>);
    ...
    Image_View_Info : aliased VkImageViewCreateInfo := (viewType         => VK_IMAGE_VIEW_TYPE_2D,
                                                        ...
                                                        subresourceRange => (levelCount     => 1,
                                                                             ...
                                                                             layerCount     => 1, 
                                                                             baseArrayLayer => 0,
                                                                             aspectMask     => VK_IMAGE_ASPECT_COLOR_BIT, others => <>), others => <>);
    
    -- Depth buffer
    Depth_Image_Barrier : aliased VkImageMemoryBarrier := (oldLayout           => VK_IMAGE_LAYOUT_UNDEFINED,
                                                           newLayout           => VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL,
                                                           srcQueueFamilyIndex => VK_QUEUE_FAMILY_IGNORED,
                                                           dstQueueFamilyIndex => VK_QUEUE_FAMILY_IGNORED,
                                                           srcAccessMask       => 0,
                                                           dstAccessMask       => VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT or
    ...
    Render_Pass_Info : aliased VkRenderPassCreateInfo := (attachmentCount => Attachments'Length,
                                                          ...
                                                          subpassCount    => 1,
                                                          pSubpasses      => Subpass_Description'Unchecked_Access,
                                                          dependencyCount => 1,
                                                          pDependencies   => Subpass_Dependency'Unchecked_Access, others => <>);
                                           
    -- Start of Initialize_Framebuffer                                       
    begin
      ...
      vkAssert (vkGetSwapchainImagesKHR (device               => Device,
                                         swapchain            => Swapchain,
                                         pSwapchainImageCount => Surface_Details.maxImageCount'Unchecked_Access,
                                         pSwapchainImages     => null));
      declare
      Images : aliased Array_Ptr (1..Int (Surface_Details.maxImageCount)) := (others => NULL_PTR);
      begin
        vkAssert (vkGetSwapchainImagesKHR (device               => Device,
                                           swapchain            => Swapchain,
                                           pSwapchainImageCount => Surface_Details.maxImageCount'Unchecked_Access,
      ...
      -- Allocate and bind the memory for the depth image
      ...
      -- Create the depth image's view
      Image_View_Info.format := Depth_Format.format;
      Image_View_Info.image  := Depth_Image.Data;
      Image_View_Info.subresourceRange.aspectMask := VK_IMAGE_ASPECT_DEPTH_BIT or VK_IMAGE_ASPECT_STENCIL_BIT;
      vkAssert (vkCreateImageView (Device, Image_View_Info'Unchecked_Access, null, Depth_Image.View'Address));
      Depth_Image_Barrier.image := Depth_Image.Data;
      
      -- Transition the depth buffers layout
      vkAssert (vkAllocateCommandBuffers (Device, Command_Buffer_Allocate_Info'Unchecked_Access, Command_Buffer'Unchecked_Access));
      vkAssert (vkBeginCommandBuffer (Command_Buffer, Command_Buffer_Begin_Info'Unchecked_Access));
      vkCmdPipelineBarrier (commandBuffer            => Command_Buffer,
                            ...
                            pBufferMemoryBarriers    => null,
                            imageMemoryBarrierCount  => 1,
                            pImageMemoryBarriers     => Depth_Image_Barrier'Unchecked_Access);
      vkAssert (vkEndCommandBuffer (Command_Buffer));
  ...
  procedure Initialize_Drawing is
    ...
    -- Application details
    Instance_Extensions : aliased Array_Ptr_Char_8_C := (C (VK_KHR_SURFACE_EXTENSION), Get_Vulkan_Extension);
    ...
    Application_Info    : aliased VkApplicationInfo  := (pApplicationName   => C (Application_Name),
                                                         ...
                                                         apiVersion         => VK_API_VERSION_1_2, others => <>);
    Instance_Info : aliased VkInstanceCreateInfo := (pApplicationInfo        => Application_Info'Unchecked_Access,
                                                     ...
                                                     ppEnabledLayerNames     => DEBUGING_EXTENSIONS (1)'Address, others => <>);
    ...
    -- Descriptor pool
    Descriptor_Pool_Sizes : aliased Array_VkDescriptorPoolSize (1..2) := (others => <>);
    Descriptor_Pool_Info  : aliased VkDescriptorPoolCreateInfo        := (poolSizeCount => Descriptor_Pool_Sizes'Length,
                                                                          ...
                                                                          flags         => VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT, others => <>); 
  
    -- Memory allocation temporaries
    Alignment_Mod        : aliased Int_64_Unsigned_C;
    Memory_Allocate_Info : aliased VkMemoryAllocateInfo := (others => <>);
    Memory_Requirements  : aliased VkMemoryRequirements := (others => <>);
    
    -- Buffer
    Staging_Buffer_Info : aliased VkBufferCreateInfo := (size        => Int_64_Unsigned (Max_Upload_Buffer.Get * MB),
                                                         usage       => VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                                                         sharingMode => VK_SHARING_MODE_EXCLUSIVE, others => <>);
                                                                           
    -- Staging fence
    Staging_Fence_Info : aliased VkFenceCreateInfo := (flags => VK_FENCE_CREATE_SIGNALED_BIT, others => <>);
    
    -- Command buffer
    Staging_Command_Pool_Info : aliased VkCommandPoolCreateInfo := (flags => VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT,
                                                                    queueFamilyIndex => Queue_Family, others => <>); 
    Staging_Command_Buffer_Allocate_Info : aliased VkCommandBufferAllocateInfo := (level => VK_COMMAND_BUFFER_LEVEL_PRIMARY,
                                                                                   commandBufferCount => 1, others => <>);
    ...
    begin
      ...
      if VkCreateInstance (Instance_Info'Unchecked_Access, null, Instance'Unchecked_Access) = VK_ERROR_LAYER_NOT_PRESENT then
        ...
        Info ("Using Vulkan's debugging layer requires the LunarG" & Char'Val (16#00AE#) & " Vulkan" & Char'Val (16#2122#) & " SDK!" & EOL
      ...
      declare Physical_Devices : aliased Array_Ptr := (1..Int (Count) => NULL_PTR); begin
        ...
        for Current_Physical_Device of Physical_Devices loop begin
          ...
          if Physical_Device = NULL_PTR 
            ...
            -- Case 3: The chosen device and the current one are similar - prefer the one with more VRAM
            or else (((Device_Properties.deviceType         = VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU and
                       Current_Device_Properties.deviceType = VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU)
      ...
      exception when others => Error ("Your GPU is incompatible!"); raise Program_Error; end;
      ...
      -- Staging fence
      vkAssert (vkCreateFence (Device, Staging_Fence_Info'Unchecked_Access, null, Staging_Fence'Access)); 

      -- Staging command pool
      vkAssert (vkCreateCommandPool (Device, Staging_Command_Pool_Info'Unchecked_Access, null, Staging_Command_Pool'Unchecked_Access));
      Staging_Command_Buffer_Allocate_Info.commandPool := Staging_Command_Pool;

      -- Staging command buffer
      vkAssert (vkAllocateCommandBuffers (Device, Staging_Command_Buffer_Allocate_Info'Unchecked_Access, Staging_Commands'Access));
      
      -- Staging staging buffer
      vkAssert (vkCreateBuffer (Device, Staging_Buffer_Info'Unchecked_Access, null, Staging_Buffer'Access));
      
      -- Staging allocation size calculations
      ...
      Memory_Allocate_Info := (memoryTypeIndex => Find_Memory_Type_Index (Memory_Requirements.memoryTypeBits, CPU_To_GPU_Usage),
                               allocationSize  => Memory_Requirements.size +
                                 (if Alignment_Mod = 0 then 0 else Memory_Requirements.alignment - Alignment_Mod), others => <>);
      ...
      vkAssert (vkMapMemory        (Device, Staging_Device_Memory, 0, Memory_Requirements.size, 0, Staging_Data'Address));
      ...
      for I in Shaders.Iterate loop
        ...
        for Stage of Shader.Stages loop
          ...
          Shader_Path := OS_Info.App_Path
                           & U (PATH_SHADERS) & Map_Shader.Unsafe.Key (I) & "-"
                           & To_Lower (Stage.Kind'Wide_Image) (1..4) & ".spv";          
          if not Exists (To_Str_8 (S (Shader_Path))) then -- Str_8!
            Error ("Missing shader stage " & S (Shader_Path));
            raise Program_Error;
          end if;
          ...
          exception when others => Error ("Error loading shader " & S (Shader_Path)); raise Program_Error; end;
             
          -- Create the state
          Shader.Stages_Info.Append ((stage => Shader_Flags, module => Stage.Program, pName => C (Shader_Entry_Name), others => <>));
            
          -- Sampler descriptors and related uniforms
          case Stage.Kind is
            when Fragment_Stage =>
              Layout_Bindings.Append ((binding        => Base_Color_Sampler.Get_Binding,
                                       descriptorType => VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 
                                       stageFlags     => Shader_Flags, others => <>));   
              Layout_Bindings.Append ((binding        => Enable_Base_Color.Get_Binding, 
                                       descriptorType => VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,         
                                       stageFlags     => Shader_Flags, others => <>));
              Layout_Bindings.Append ((binding        => Specular_Sampler.Get_Binding,   
                                       descriptorType => VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 
                                       stageFlags     => Shader_Flags, others => <>));
              Layout_Bindings.Append ((binding        => Normal_Sampler.Get_Binding,     
                                       descriptorType => VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 
                                       stageFlags     => Shader_Flags, others => <>));
              Layout_Bindings.Append ((binding        => Enable_Specular.Get_Binding,    
                                       descriptorType => VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,         
                                       stageFlags     => Shader_Flags, others => <>));
              Layout_Bindings.Append ((binding        => Enable_Normal.Get_Binding,      
                                       descriptorType => VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,         
                                       stageFlags     => Shader_Flags, others => <>));
        ...
        Layout_Bindings.Append ((binding        => Enable_Skinning.Get_Binding,
                                 descriptorType => VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
                                 stageFlags     => VK_SHADER_STAGE_Vertex_BIT or VK_SHADER_STAGE_FRAGMENT_BIT, others => <>));
        ...
        -- Overwrite the initial shader with the fully loaded one
        Shaders.Replace_Element (I, Shader);
      end loop;       
      
      -- Kick off the second stage!
      Initialize_Framebuffer;
    exception when others => Error ("Vulkan driver not installed or up to date?"); raise Program_Error; end;
  ...
  -- Main drawing routine reponsible for managing pipelines
  procedure Draw (Data : Bottom_Level_State; Frame : in out Framebuffer_State; Surface_Sort : Surface_Sort_Array) is
    --Default_Material    :         Material_State              := Materials.Get ("default_base_color");
    ...
    procedure Stage_Image (Path        : Texture_Name_Str.T;
      ...
      begin
        if Path.Current_Length = 0 then Set_Flag (False); return; end if;
        Set_Flag (True);
        Set_Sampler (To_Str_16 (Path) & S (Hash));
      end;
...
--        end if;
      ...
      for Segment of Data.Mesh.Segments loop
        Material := Materials.Get (S (Segment.Material));
        
        -- Optimize away unneeded material passes
        if Surface_Sort (Material.Sort) then
          ...
          if Index (S (Segment.Material), "decal") /= 0 then
            goto End_Of_Loop;
          end if;
          Hash := Get_Material_Info_Hash (Material);
...
goto Have_Pipeline;
          ...
          Viewport : aliased VkViewport := (x        => 0.0,
                                            ...
                                            minDepth => 0.0,
                                            maxDepth => 1.0,
                                            width    => Real_C (Surface_Extent.width),
                                            height   => Real_C (Surface_Extent.height), others => <>);
          Scissor : aliased VkRect2D := (offset => (0, 0), extent => Surface_Extent, others => <>);
          ...
          Input_Assembly_Info : aliased VkPipelineInputAssemblyStateCreateInfo := (topology => VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST,
                                                                                   primitiveRestartEnable => VK_FALSE, others => <>);                                                 
          Multisample_Info    : aliased VkPipelineMultisampleStateCreateInfo := (sampleShadingEnable => VK_FALSE,
                                                                                 rasterizationSamples => VK_SAMPLE_COUNT_1_BIT, others => <>);
                                                               --rasterizationSamples => Image_Properties.sampleCounts,
          ...
          Vertex_Input_Info : aliased VkPipelineVertexInputStateCreateInfo :=
            (vertexBindingDescriptionCount   => 1,
             pVertexBindingDescriptions      => VERTEX_BINDING'Unchecked_Access,
             vertexAttributeDescriptionCount => VERTEX_ATTRIBUTES'Length,
             pVertexAttributeDescriptions    => VERTEX_ATTRIBUTES (1)'Unchecked_Access, others => <>);   
          ...
          Depth_Stencil_Info : aliased VkPipelineDepthStencilStateCreateInfo :=
            (depthTestEnable       => VK_TRUE,
             ...
             minDepthBounds        => 0.0,
             maxDepthBounds        => 0.0,
             stencilTestEnable     => To_VkBool32 ((Pipeline.Front_Fail or Pipeline.Front_Pass or Pipeline.Front_Depth_Fail) /= 0
                                                     or (if not Pipeline.Has_Back_Stencil then False
                                                         else (Pipeline.Back_Fail
                                                                or Pipeline.Back_Pass
                                                                or Pipeline.Back_Depth_Fail) /= 0)),
             ...
             back                  => (writeMask   => 16#FFFF_FFFF#,
                                       ...
                                       compareMask => Pipeline.Compare_Mask,
                                       reference   => Pipeline.Stencil_Reference,
                                       failOp      => (if Pipeline.Has_Back_Stencil then Pipeline.Back_Fail      
                                                       else Pipeline.Front_Fail),
                                       passOp      => (if Pipeline.Has_Back_Stencil then Pipeline.Back_Pass       
                                                       else Pipeline.Front_Pass),
                                       depthFailOp => (if Pipeline.Has_Back_Stencil then Pipeline.Back_Depth_Fail 
                                                       else Pipeline.Front_Depth_Fail)), others => <>); 
          ...
          Color_Blend_Attachment : aliased VkPipelineColorBlendAttachmentState :=
            (srcColorBlendFactor => Pipeline.Source_Blend_Factor,
             ...
             colorWriteMask      =>  VK_COLOR_COMPONENT_R_BIT or
                                     ...
                                     VK_COLOR_COMPONENT_A_BIT, others => <>);
             --Pipeline.Color_Mask, others => <>);
          ...
          Dynamic_State_Info : aliased VkPipelineDynamicStateCreateInfo := (pDynamicStates    => Dynamic_States (1)'Unchecked_Access,
                                                                            dynamicStateCount =>
                                                                              (if Pipeline.Has_Polygon_Offset then 1 else 0) +
...
<<Have_Pipeline>>       
          ...
          vkAssert (vkAllocateDescriptorSets (Device, Descriptor_Set_Info'Unchecked_Access, Frame.Descriptor_Set'Unchecked_Access));
          if Writes.Length > 0 then
            ...
            begin     
              for I in Writes_Internal'Range loop
                ...
                Resulting_Writes (I).dstSet := Frame.Descriptor_Set;
                if Writes_Internal (I).Is_Image then
                  Resulting_Writes (I).pImageInfo  := Writes_Internal (I).Image_Info'Address;
                else
                  Resulting_Writes (I).pBufferInfo := Writes_Internal (I).Buffer_Info'Address;
                end if;
              end loop;
          ...
          -- Bind everything
          vkCmdBindPipeline       (Frame.Commands, VK_PIPELINE_BIND_POINT_GRAPHICS, Current_Pipeline); 
          vkCmdBindDescriptorSets (commandBuffer      => Frame.Commands, 
                                   pipelineBindPoint  => VK_PIPELINE_BIND_POINT_GRAPHICS, 
                                   layout             => Pipeline.Shader.Pipeline_Layout,
                                   firstSet           => 0,
                                   descriptorSetCount => 1,
                                   pDescriptorSets    => Frame.Descriptor_Set'Unchecked_Access, 
                                   dynamicOffsetCount => 0,
                                   pDynamicOffsets    => null);
          vkCmdBindVertexBuffers  (commandBuffer      => Frame.Commands,
                                   firstBinding       => 0,
                                   bindingCount       => 1,
                                   pBuffers           => Data.Mesh.Vertices.Data'Address,
                                   pOffsets           => Bind_Vertex_Offset'Address);
          vkCmdBindIndexBuffer    (commandBuffer      => Frame.Commands,
                                   buffer             => Segment.Indices.Data,
                                   offset             => 0,
                                   indexType          => VK_INDEX_TYPE_UINT32);
                                   
          -- Draw
          vkCmdDrawIndexed (Frame.Commands, Int_Unsigned_C (Segment.Indices.Count), 1, 0, 0, 0); -- Data.Mesh.Segment_Count    
        end if;
        
<<End_Of_Loop>>
      end loop;
          
      -- Kill the current Descriptor_Set
      Writes.Clear;
    end;    
end;
